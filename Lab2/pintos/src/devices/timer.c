...
//   thread_current()->wakeupTime = wakeTime;

//   // 2. Put the current thread into the list of sleeping threads

//   // 3. Call whatever function puts the current thread to sleep
...
/* 
 * Check if thread is sleeping, if so, wake it up.
 * Called from timer_interupt handler in thread_foreach 
 */
static void 
wake_sleeping_threads(struct thread *t, void *aux UNUSED)
{
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  // Look at list of sleeping threads, for each one:
  // Check if it needs to be woken up
  // if so, wake it up.
  // Alternatively, look through list of all threads.
  // (maybe) create wakeUpSleepingThreads function
  // call it using thread_foreach
